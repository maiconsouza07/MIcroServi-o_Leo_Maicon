...
exports.registrarEmprestimo = async (req, res) => {
  try {
    ...
    const livroResponse = await axios.get(`http://localhost:3002/livros/${livroId}`);
    ...
    await axios.patch(`http://localhost:3002/livros/${livroId}`, { disponivel: false });
...
exports.registrarDevolucao = async (req, res) => {
  try {
    ...
    await axios.patch(`http://localhost:3002/livros/${emprestimo.livroId}`, { disponivel: true });
